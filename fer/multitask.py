...
class CoVWeightingLoss(nn.Module):
    ...
    def __init__(self, num_losses, mean_decay=None):
        ...
        self.running_mean_L = torch.zeros((self.num_losses,), requires_grad=False).type(
            torch.FloatTensor
        )
        self.running_mean_l = torch.zeros((self.num_losses,), requires_grad=False).type(
            torch.FloatTensor
        )
        self.running_S_l = torch.zeros((self.num_losses,), requires_grad=False).type(
            torch.FloatTensor
        )
    ...
    def forward(self, unweighted_losses):
        ...
        if self.current_iter <= 1:
            self.alphas = (
                torch.ones((self.num_losses,), requires_grad=False).type(
                    torch.FloatTensor
                )
                / self.num_losses
            )
